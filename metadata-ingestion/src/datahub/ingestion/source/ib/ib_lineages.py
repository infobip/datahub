...
class IBLineagesSource(IBRedashSource):
    ...
    def get_workunits(self) -> Iterable[Union[MetadataWorkUnit, UsageStatsWorkUnit]]:
        lineages_grouped = pd.read_json(json.dumps(self.query_get(self.config.query_id))).groupby(
        ...
        for key_tuple, lineages in lineages_grouped:
            ...
            dst_dataset_urn = build_dataset_urn(first.DstType, first.DstTableName,
            ...
            for index, row in lineages.iterrows():
                src_urns.append(
                    build_dataset_urn(row.SrcType, row.SrcTableName,
                                      row.SrcLocationCode,
                                      row.SrcDBName,
                                      row.SrcSchemaName)
                )
